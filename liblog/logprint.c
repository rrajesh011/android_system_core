...
LIBLOG_ABI_PUBLIC int android_log_shouldPrintLine(AndroidLogFormat* p_format,
                                                  const char* tag,
                                                  android_LogPriority pri) {
    if (!strncmp(tag, "QCamera", 7) || !strncmp(tag, "WCNSS_FILTER", 12) || !strncmp(tag, "mm-3a-core", 10) || !strncmp(tag, "AEC_PORT", 8) || !strncmp(tag, "mm-camera", 9) || !strncmp(tag, "GalleryDatab", 12) || !strncmp(tag, "ANDR-PERF", 9))
        return 0;
    else
